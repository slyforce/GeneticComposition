...
def conv_layer(x, scope, initializer=None, filters=64, stride=(1,1), kernel_size=(3,3), name=''):
    with tf.variable_scope(scope):
        x = tf.layers.conv2d(x, kernel_initializer=initializer, filters=filters,
                             kernel_size=kernel_size, strides=stride, name=name)
    return x
...
def deconv2d(input_, output_shape,
    ...
    with tf.variable_scope(variable_scope) as vs:
        relu_initializer = tf.contrib.layers.variance_scaling_initializer(factor=2.0, mode='FAN_IN', uniform=False,
...
def visualize_cnn_kernel(kernel_name, tensorboard_name):
    kernel = [v for v in tf.global_variables() if str(v.name).startswith(kernel_name)][0]
    # shape  = kernelsize x in_channel x out_channel
    kernel = tf.transpose(kernel, [2, 3, 0, 1])
    kernel = tf.reshape(kernel, [kernel.shape[0] * kernel.shape[1], kernel.shape[2], kernel.shape[3]])
...
def MIDINetDiscriminator(x, feature_size, is_training=True, reuse=False):
    with tf.variable_scope("Discriminator", reuse=reuse) as vs:
        relu_initializer = tf.contrib.layers.variance_scaling_initializer(factor=2.0, mode='FAN_IN', uniform=False, seed=None, dtype=tf.float32)

        x = conv_layer(x, vs, filters=14, stride=(2,2), kernel_size=(2,feature_size), name='d-conv-1')
        x = tf.nn.leaky_relu(x, alpha=0.2)
        #x = batch_norm(vs, x, is_training, 'd-batch-norm-1')

        x = conv_layer(x, vs, filters=24, stride=(2,2), kernel_size=(2, 1), name='d-conv-2')
        x = tf.nn.leaky_relu(x, alpha=0.2)
...
def MIDINetGenerator(batch_size, sequence_length, feature_size, reuse=False, is_training=True):
    ...
    with tf.variable_scope("Generator", reuse=reuse) as vs:
        ...
        x = tf.layers.dense(x, int(np.ceil(sequence_length/8.)) * int(np.ceil(feature_size/8.)),
        ...
        x = tf.reshape(x, shape=[batch_size, int(np.ceil(sequence_length/8.)), int(np.ceil(feature_size/8.)), 1])

        print "shape of initial image", x.shape

        x = deconv2d(x, [batch_size, int(np.ceil(sequence_length/4.)), int(np.ceil(feature_size/4.)), 128], vs, 2, 1,
                     2, 2, 'g-deconv-1')
        #x = batch_norm(vs, x, is_training, 'g-batch-norm-1')
        x = tf.nn.leaky_relu(x, alpha=0.2)
        print "after deconv1", x.shape

        x = deconv2d(x, [batch_size, int(np.ceil(sequence_length/2)), int(np.ceil(feature_size/2.)), 128], vs, 2, 1,
                     2, 2, 'g-deconv-2')
        #x = batch_norm(vs, x, is_training, 'g-batch-norm-2')
        x = tf.nn.leaky_relu(x, alpha=0.2)
        print "after deconv2", x.shape

        x = deconv2d(x, [batch_size, sequence_length, feature_size, 128], vs, 2, 1,
                     2, 2, 'g-deconv-3')
        #x = batch_norm(vs, x, is_training, 'g-batch-norm-3')
        x = tf.nn.leaky_relu(x, alpha=0.2)
        print "after deconv3", x.shape

        x = deconv2d(x, [batch_size, sequence_length, feature_size, 1], vs, 2, feature_size, 1, 1, 'g-deconv-4')
        ...
        if reuse == False:
            tf.summary.image('generator_image_final', tf.transpose(x, [0,2,1,3]), max_outputs=1)
...
def MIDINetGeneratorFFNN(batch_size, sequence_length, feature_size, is_training=False, reuse=False):
    with tf.variable_scope("Generator", reuse=reuse) as vs:
        ...
        x = tf.layers.dense(x, sequence_length * feature_size, activation=None, name='g_output_layer')
...
def MIDINetGeneratorSampleInput(input, input_classes, batch_size, sequence_length, feature_size, reuse=False,
                                is_training=True):
    # input has a format of [B,H,W,Ch]
    with tf.variable_scope("Generator", reuse=reuse) as vs:

        # encoding of the conditioning sequence
        h1 = tf.layers.conv2d(input, kernel_size=(2,1), strides=(2,2), filters=10, name='input-conv-1',
                              padding='SAME')
        h1 = tf.nn.leaky_relu(h1, alpha=0.2)

        h2 = tf.layers.conv2d(h1, kernel_size=(2,1), strides=(2,2), filters=10, name='input-conv-2',
                              padding='SAME')
        h2 = tf.nn.leaky_relu(h2, alpha=0.2)

        h3 = tf.layers.conv2d(h2, kernel_size=(2,1), strides=(2,2), filters=10, name='input-conv-3',
                              padding='SAME')
        ...
        x = tf.reshape(x,
                       shape=[batch_size, int(np.ceil(sequence_length/8.)), int(np.ceil(feature_size/8.)), 1])
